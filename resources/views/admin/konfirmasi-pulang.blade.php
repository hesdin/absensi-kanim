...
@section('title', 'Konfirmasi Pulang')
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h5 class="card-title">Konfirmasi Pulang</h5>
                    </div>
                </div>
                            ...
                            <tbody>
                                ...
                                @forelse ($absens as $absen )
                                    <tr>
                                        ...
                                        <td>{{ Carbon\Carbon::parse($absen->date_pulang)->format('Y/m/d') }}</td>
                                        <td>{{ Carbon\Carbon::parse($absen->date_pulang)->format('H:i') }}</td>
                                        <td>
                                            <a target="_blank" class="btn btn-sm btn-icon text-primary flex-end" data-bs-toggle="tooltip" title="" href="{{ route('admin.foto-pulang', $absen->id) }}" data-bs-original-title="Lihat Foto">
                                        ...
                                        <td>
                                            ...
                                            <a class="btn btn-sm btn-icon text-success flex-end" data-bs-toggle="tooltip" title="" href="{{ route('admin.terima-kehadiran', ['status' =>'pulang', 'id' => $absen->id]) }}" data-bs-original-title="Terima">
                                                <span class="btn-inner">
                                            ...
                                            <a class="btn btn-sm btn-icon text-danger " data-bs-toggle="tooltip" title="" href="{{ route('admin.tolak-kehadiran', ['status' =>'pulang', 'id' => $absen->id]) }}" onclick="event.preventDefault();document.getElementById('tolak-kehadiran').submit();" data-bs-original-title="Tolak">
                                                <form action="{{ route('admin.tolak-kehadiran', ['status' =>'pulang', 'id' => $absen->id]) }}" method="POST" class="d-none" id="tolak-kehadiran">@csrf @method('DELETE')</form>
                                                <span class="btn-inner">
